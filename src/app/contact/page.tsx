...
export default function ContactPage() {
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try { const r = await fetch("/api/contact", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ name:fd.get("name"), email:fd.get("email"), subject:fd.get("subject"), message:fd.get("message") }) }); if (!r.ok) throw new Error((await r.json()).error||"Failed"); setDone(true); }
  ...
  return (<main className="min-h-screen pt-16"><Navbar />
    <section className="py-24 px-6 hero-bg relative"><div className="absolute top-[-10%] right-[15%] w-[350px] h-[350px] bg-violet-100/30 rounded-full blur-[100px]" />
      <div className="relative z-10 max-w-[850px] mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <ScrollReveal><div className="card p-7">
            {done ? <div className="text-center py-10"><div className="w-14 h-14 rounded-full bg-emerald-50 flex items-center justify-center mx-auto mb-3"><svg className="w-7 h-7 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg></div><h3 className="text-xl font-bold mb-1">Sent!</h3><p className="text-slate-500">We&apos;ll reply soon.</p></div>
          ...
          <ScrollReveal delay={150}><div className="space-y-4">
            ...
            <div className="card p-5 bg-indigo-50 border-indigo-100"><h3 className="font-bold text-[14px] mb-1">Want to join us?</h3><p className="text-slate-500 text-[13px] mb-2">Check our open positions.</p>
