...
export default function CareersPage() {
  return (<main className="min-h-screen pt-16"><Navbar />
    <section className="py-24 px-6 hero-bg relative"><div className="absolute top-[-10%] right-[10%] w-[350px] h-[350px] bg-emerald-100/30 rounded-full blur-[100px]" />
    ...
    <section className="py-20 px-6 bg-soft"><div className="max-w-5xl mx-auto">
    ...
    <section id="jobs" className="py-20 px-6 bg-white"><div className="max-w-[850px] mx-auto">
      ...
      <div className="space-y-3">
        {jobs.map((j,i) => (<ScrollReveal key={j.title} delay={i*60}><div className="card p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div><div className="flex items-center gap-2 mb-1"><div className={`w-2 h-2 rounded-full bg-gradient-to-r ${j.g}`} /><h3 className="font-bold">{j.title}</h3></div>
            <p className="text-slate-500 text-[13px]">{j.desc}</p>
            <div className="flex gap-2 mt-2"><span className="text-[10px] font-semibold bg-slate-100 text-slate-500 px-2 py-0.5 rounded">{j.type}</span><span className="text-[10px] font-semibold bg-slate-100 text-slate-500 px-2 py-0.5 rounded">Remote</span><span className={`text-[10px] font-semibold text-white px-2 py-0.5 rounded bg-gradient-to-r ${j.g}`}>{j.team}</span></div>
          </div>
